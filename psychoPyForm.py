...
expName = 'third'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\julia\\Documents\\Perso\\Projet Stégano\\third.py',
...
instructions = visual.TextStim(win=win, name='instructions',
    text='Please make sure your sound is switch on\n\nPress space to continue',
    font='Arial',
...
sound_sampl = sound.Sound('A', secs=1.0, stereo=True, hamming=True,
    name='sound_sampl')
sound_sampl.setVolume(1)
...
Instruc = visual.TextStim(win=win, name='Instruc',
    text='What do you think about this sound ?',
    font='Arial',
    pos=(0, 0.3), height=0.05, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
    depth=-1.0);
answer_ = visual.TextBox2(
...
button = visual.ImageStim(
    win=win,
    name='button', 
    image='buttonContinue.png', mask=None,
    ori=0, pos=(0.75, -0.35), size=(0.2,0.1),
...
thank = visual.TextStim(win=win, name='thank',
    text='Thank you for your participation to our trial',
    font='Arial',
...
InstructionsComponents = [welcome, instruction_resp, instructions]
...
for thisTrial in trials:
    ...
    # update component parameters for each repeat
    sound_sampl.setSound(soundSample, secs=1.0, hamming=True)
    sound_sampl.setVolume(1, log=False)
    # keep track of which components have finished
    TrialComponents = [sound_sampl]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        # start/stop sound_sampl
        if sound_sampl.status == NOT_STARTED and t >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            sound_sampl.frameNStart = frameN  # exact frame index
            sound_sampl.tStart = t  # local t and not account for scr refresh
            sound_sampl.tStartRefresh = tThisFlipGlobal  # on global time
            sound_sampl.play()  # start the sound (it finishes automatically)
        if sound_sampl.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > sound_sampl.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
                sound_sampl.tStop = t  # not accounting for scr refresh
                sound_sampl.frameNStop = frameN  # exact frame index
                win.timeOnFlip(sound_sampl, 'tStopRefresh')  # time at next scr refresh
                sound_sampl.stop()
    ...
    for thisComponent in TrialComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    sound_sampl.stop()  # ensure sound has stopped at end of routine
    trials.addData('sound_sampl.started', sound_sampl.tStart)
    trials.addData('sound_sampl.stopped', sound_sampl.tStop)
    ...
    answer.rt = []
    _answer_allKeys = []
    answer_.setText('Enter your answer here')
    # setup some python lists for storing info about the mouse
    mouse.clicked_name = []
    gotValidClick = False  # until a click is received
    # keep track of which components have finished
    AnswerComponents = [answer, Instruc, answer_, button, mouse]
    ...
    while continueRoutine:
        ...
        if answer.status == STARTED and not waitOnFlip:
            theseKeys = answer.getKeys(keyList=['ctrl+enter'], waitRelease=False)
        ...
        # *answer_* updates
        if answer_.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            answer_.frameNStart = frameN  # exact frame index
            answer_.tStart = t  # local t and not account for scr refresh
            answer_.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(answer_, 'tStartRefresh')  # time at next scr refresh
            answer_.setAutoDraw(True)
    ...
    trials.addData('Instruc.started', Instruc.tStartRefresh)
    trials.addData('Instruc.stopped', Instruc.tStopRefresh)
    trials.addData('answer_.text',answer_.text)
    trials.addData('answer_.started', answer_.tStartRefresh)
    trials.addData('answer_.stopped', answer_.tStopRefresh)
...
_key_resp_allKeys = []
# keep track of which components have finished
ThanksComponents = [thank, key_resp]
